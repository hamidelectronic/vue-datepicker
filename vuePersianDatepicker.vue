<style>
  ...
  .hd-times {
    ...
    border-radius: 12px;
    box-shadow: 0 10px 30px -10px rgba(0, 0, 0, 0.23);
    display: flex;
    align-items: center;
    ...
    <div class="datepicker-overlay" v-if="showInfo.check" @click="dismiss($event)" v-bind:style="{'background' : option.overlayOpacity? 'rgba(0,0,0,'+option.overlayOpacity+')' : 'rgba(0,0,0,0.5)'}">
      <div class="cov-date-body" :style="covDateBody">
        ...
        <div class="button-box">
          ...
          <span @click="picked">{{(!selectedHours && option.type == 'multi-day') ? (option.buttons ? option.buttons.next : 'Next') : (option.buttons ? option.buttons.ok : 'Ok')}}</span>
        </div>
      </div>
    </div>
  </div>
    ...
    exports.default = {
        ...
        data: function data() {
            ...
            return {
                ...
                library: {
                    week: this.option.jalaali ? this.option.jWeek : this.option.week,
                    month: this.option.jalaali ? this.option.jMonth : this.option.month,
                    year: []
                },
                checked: {
                    oldtime: '',
        ...
        methods: {
            ...
            showDay: function showDay(time) {
                ...
                var firstDay = this.option.jalaali ? this.option.week.indexOf((0, _moment2.default)(currentMoment, 'jYYYY-jMM-jDD HH:mm:ss').format('dd'))+1 : (0, _moment2.default)(currentMoment).date(1).day();
            ...
            showYear: function showYear() {
                ...
                this.$nextTick(function () {
                    ...
                    listDom.scrollTop = listDom.scrollHeight - 900;
                    //_this3.addYear();
                });
            },
            ...
            setYear: function setYear(year) {
                this.checked.currentMoment = (0, _moment2.default)(year + '-' + this.checked.month + '-' + this.checked.day).format('YYYY-MM-DD HH:mm:ss');
            ...
            setMonth: function setMonth(month) {
                ...
                this.checked.currentMoment = (0, _moment2.default)(this.checked.year + '-' + mo + '-' + this.checked.day).format('YYYY-MM-DD HH:mm:ss');
                this.showDay(this.checked.currentMoment);
            },
            ...
            picked: function picked() {
                if(this.selectedDays.length <= 0 && this.option.type === 'multi-day') {
                    return;
                }
                switch (this.option.type) {
                    case 'multi-day':
            ...
            pickedS: function picked() {
                if (this.option.type === 'day' || this.option.type === 'min') {
                    ...
                    this.selectedDays.forEach((value, index) => {
                        var ctime = (0, _moment2.default)(value + ' ' + this.checked.hour + ':' + this.checked.min , 'YYYY-MM-DD HH:mm:ss');
                        this.$set(this.selectedDays, index, (0, _moment2.default)(ctime).format(this.option.format));
                        //newSelectedDays[index] = (0, _moment2.default)(ctime).format(this.option.format);
                        //this.times_selected.push({date: value, time: this.checked.hour + ':' + this.checked.min + ":00", en_date: (0, _moment2.default)(ctime).format(this.option.format)});
                    });
